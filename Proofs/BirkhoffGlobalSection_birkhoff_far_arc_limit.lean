...
import Theorems.Thm_BirkhoffGlobalSection_birkhoff_far_no_rest_end
...
/-- The far family at its first bad parameter converges to a collision orbit or a vertical
tangency, via Birkhoff's identity (59); the only extra input excludes an end at rest. -/
theorem far_arc_limit2 (μ c : ℝ)
    (hμ0 : 0 < μ) (hμ1 : μ < 1)
    (hc : belowFirstCriticalValue μ c)
    (φ : Flow ℝ (LeftEnergyState μ c))
    (hφ : IsLeviCivitaHamiltonianFlow μ c φ)
 ...
 := by
  ...
  · -- below the axis on the vertical line
    ...
    have hjv1 := birkhoff_far_no_rest_end μ c hμ0 hμ1 hc φ hφ b hb hgood hbad xb hxb S hS0
      (fun t ht => ⟨by have := (hopen t ht).1; simp only [hX, hγd] at this; exact this,
        hyS t ht, hvxS t ht⟩) hXS0' hYS' hjv0
    refine ⟨xb, hxb, by rw [(jv_eq μ _).2]; exact hv, S, hS0, fun t ht => (hyS t ht).le, hvxS,
      fun t ht => by rw [← hγd]; exact hPof t (hopen t ht).2.1, hXS0', hYS'.le, hjv0.symm.le,
      fun _ _ => hjv1, ?_⟩
    intro η hη
...
theorem solution (μ c : ℝ)
    ...
    ∃ xb : LeftEnergyState μ c, (xb : Phase) = farShootingStart μ c b ∧
      ...
      ∀ η > 0, ∃ δ > 0, ∀ r ∈ Set.Ioo (b - δ) b, ∀ x : LeftEnergyState μ c, ∀ τ : ℝ,
        (x : Phase) = farShootingStart μ c r → IsFarShootingArc φ x τ → |τ - T| < η
 :=
  far_arc_limit2 μ c hμ0 hμ1 hc φ hφ b hb hgood hbad
