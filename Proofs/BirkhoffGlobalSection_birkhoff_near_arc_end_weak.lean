...
import Theorems.Thm_BirkhoffGlobalSection_birkhoff_near_collision_strict
import Theorems.Thm_BirkhoffGlobalSection_birkhoff_near_arc_degeneration
...
/-- The crossing state of a near arc lies on the diagonal `z = (α, -α)` with `α > 0`. -/
lemma near_cross_state (μ c : ℝ) (φ : Flow ℝ (LeftEnergyState μ c))
    (hφ : IsLeviCivitaHamiltonianFlow μ c φ) (x : LeftEnergyState μ c) (τ : ℝ)
    (hx0 : 0 < (x : Phase) 0) (harc : IsNearShootingArc φ x τ) :
    0 < ((φ (-τ) x : LeftEnergyState μ c) : Phase) 0 ∧
      ((φ (-τ) x : LeftEnergyState μ c) : Phase) 1 = -((φ (-τ) x : LeftEnergyState μ c) : Phase) 0 := by
  obtain ⟨hτ, hq, hend, hcross⟩ := harc
  have hpos : 0 < ((φ (-τ) x : LeftEnergyState μ c) : Phase) 0 := by
    by_contra hneg
    push Not at hneg
    have hcont : ContinuousOn (fun u => ((φ u x : LeftEnergyState μ c) : Phase) 0) (Icc (-τ) 0) :=
    ...
    have hmem : (0:ℝ) ∈ Icc ((fun u => ((φ u x : LeftEnergyState μ c) : Phase) 0) (-τ))
        ((fun u => ((φ u x : LeftEnergyState μ c) : Phase) 0) 0) :=
      ⟨hneg, by simp only; rw [h0]; exact hx0.le⟩
    obtain ⟨u, hu, hu0⟩ := intermediate_value_Icc (by linarith) hcont hmem
    simp only at hu0
    ...
    · have := (hq u ⟨h, lt_of_le_of_ne hu.2 hu'⟩).1
  ...
  have h1 : ((φ (-τ) x : LeftEnergyState μ c) : Phase) 1 < 0 := by
...
/-- The crossing-coordinate value at a collision velocity `(w₀, w₁)` with `w₀ > 0 > w₁`. -/
lemma Gnear_collision_bounds (μ w0 w1 : ℝ) (h0 : 0 < w0) (h1 : w1 < 0) :
    -1 ≤ Gnear μ 0 w0 w1 ∧ Gnear μ 0 w0 w1 < -(Real.sqrt 2 / 2) := by
  have e : Gnear μ 0 w0 w1 = (w1 - w0) / Real.sqrt ((w0 + w1) ^ 2 + (w1 - w0) ^ 2) := by
    unfold Gnear; congr 1 <;> ring_nf
  rw [e]
  set X := (w0 + w1) ^ 2 + (w1 - w0) ^ 2 with hX
  have hXpos : 0 < X := by
    have : 0 < (w1 - w0) ^ 2 := by nlinarith
  ...
  set a := (w1 - w0) / D with ha
  have haD : a * D = w1 - w0 := by rw [ha]; field_simp
  have hneg : a < 0 := div_neg_of_neg_of_pos (by linarith) hDpos
  have hsq : a ^ 2 * X = (w1 - w0) ^ 2 := by rw [← hD2, ← mul_pow, haD]
  constructor
  · by_contra h; push Not at h
    have : 1 < a ^ 2 := by nlinarith
    nlinarith [sq_nonneg (w0 + w1)]
  · have h2 : 1 / 2 < a ^ 2 := by
      by_contra h; push Not at h
...
theorem solution (μ c : ℝ)
    ...
    ∃ α : ℝ, -1 ≤ α ∧ α < -(Real.sqrt 2 / 2) ∧
      ∀ η > 0, ∃ δ > 0, ∀ r ∈ Set.Ioo (b - δ) b, ∀ x : LeftEnergyState μ c, ∀ τ : ℝ,
        (x : Phase) = nearShootingStart μ c r → IsNearShootingArc φ x τ →
          dist (shootingCoordinates μ ((φ (-τ) x : LeftEnergyState μ c) : Phase)) (α, 0) < η := by
  obtain ⟨xb, hxb, T, hT, hq, hc0, hc1, hconv⟩ :=
    birkhoff_near_arc_degeneration μ c hμ0 hμ1 hc φ hφ b hb hgood hbad
  ...
  obtain ⟨hw0, hw1⟩ := birkhoff_near_collision_strict μ c hμ0 hμ1 hc φ hφ xb T hT hb1 hb2
    (by rw [hb0]; exact hb) hq hc0 hc1
  set s₀ : Phase := ((φ (-T) xb : LeftEnergyState μ c) : Phase) with hs₀
  set w0 := s₀ 2
  set w1 := s₀ 3
  obtain ⟨hα1, hα2⟩ := Gnear_collision_bounds μ w0 w1 hw0 hw1
  refine ⟨Gnear μ 0 w0 w1, hα1, hα2, fun η hη => ?_⟩
  -- continuity of the crossing-coordinate formula at the collision state
  set G : Phase → ℝ × ℝ := fun s => (Gnear μ (s 0) (s 2) (s 3), 4 * s 0 ^ 2) with hG
  have hGc : ContinuousAt G s₀ := by
    have hden : Real.sqrt ((s₀ 2 + s₀ 3 + 8 * s₀ 0 ^ 3) ^ 2 + (s₀ 3 - s₀ 2 - 2 * s₀ 0 * μ) ^ 2) ≠ 0 := by
      apply (Real.sqrt_pos.2 _).ne'
      rw [hc0]
      have : 0 < (s₀ 3 - s₀ 2) ^ 2 := by
        have : s₀ 3 - s₀ 2 < 0 := by linarith
        nlinarith
      nlinarith [sq_nonneg (s₀ 2 + s₀ 3 + 8 * 0 ^ 3)]
    apply ContinuousAt.prodMk
    · unfold Gnear
      exact ContinuousAt.div (by fun_prop)
        ((by fun_prop : ContinuousAt (fun s : Phase => (s 2 + s 3 + 8 * s 0 ^ 3) ^ 2 +
          (s 3 - s 2 - 2 * s 0 * μ) ^ 2) s₀).sqrt) hden
    · exact (continuous_const.mul ((continuous_apply 0).pow 2)).continuousAt
  have hGs₀ : G s₀ = (Gnear μ 0 w0 w1, 0) := by
    rw [show G s₀ = (Gnear μ (s₀ 0) w0 w1, 4 * s₀ 0 ^ 2) from rfl, hc0]; norm_num
  ...
  have hGF : ContinuousAt (G ∘ F) (-T, xb) := hGc.comp hFc.continuousAt
  obtain ⟨ε, hε, hball⟩ := Metric.continuousAt_iff.1 hGF η hη
  -- the start moves continuously
  have hrad : 2 * b ^ 2 < 1 := by
    have := comp_radius μ c hμ0 hμ1 hc _ xb.2
    rw [hb0, hb1] at this
    nlinarith [sq_nonneg b]
  obtain ⟨δ1, hδ1, hN⟩ := Metric.continuousAt_iff.1 (nearStart_continuousAt μ c b hrad.ne) ε hε
  ...
  have hx0 : 0 < (x : Phase) 0 := by rw [hx]; simp [nearShootingStart]; exact hr0
  obtain ⟨hcpos, hcdiag⟩ := near_cross_state μ c φ hφ x τ hx0 harc
  have hcoord : shootingCoordinates μ ((φ (-τ) x : LeftEnergyState μ c) : Phase) = G (F (-τ, x)) := by
    rw [coords_near_cross μ _ _ hcpos rfl hcdiag]
  rw [hcoord, ← hGs₀]
  apply hball
  have hd1 : dist (-τ) (-T) < ε := by
    rw [Real.dist_eq, show -τ - -T = -(τ - T) by ring, abs_neg]; exact hτ r hrδ2 x τ hx harc
  have hd2 : dist x xb < ε := by
