...
theorem solution (μ c : ℝ)
    ...
    ∀ t ∈ Set.Ioo 0 T, relativePosition μ ((φ t x : LeftEnergyState μ c) : Phase) 1 < 0 := by
  exact far_axis_exclusion μ c hμ0 hμ1 hc φ hφ x T hT hx0 hx1
    (by rw [← (jv_eq μ _).2]; exact hv) hcl hvx hP
