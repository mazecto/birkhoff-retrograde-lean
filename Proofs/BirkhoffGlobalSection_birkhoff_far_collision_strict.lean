...
-- ===== Solutions.CM16 =====
set_option maxHeartbeats 4000000 in
/-- At a collision whose Levi-Civita velocity is tangent to one of the coordinate axes, the
physical vertical velocity `v₂` vanishes to first order along the orbit. -/
lemma coll_Vy_small (μ c : ℝ) (hμ0 : 0 < μ) (hμ1 : μ < 1) (hc : belowFirstCriticalValue μ c)
    (φ : Flow ℝ (LeftEnergyState μ c)) (hφ : IsLeviCivitaHamiltonianFlow μ c φ)
    (x : LeftEnergyState μ c) (h0 : (x : Phase) 0 = 0) (h1 : (x : Phase) 1 = 0)
    (htan : (x : Phase) 2 = 0 ∨ (x : Phase) 3 = 0) :
    ∃ K > 0, ∃ s1 > 0, ∀ u : ℝ, 0 < |u| → |u| ≤ s1 →
      |Vy μ ((φ u x : LeftEnergyState μ c) : Phase)| ≤ K * |u| := by
  obtain ⟨M, hM0, hM⟩ := vf_bound μ c hμ0 hμ1 hc
  obtain ⟨C, hC0, hC⟩ := coll_bounds μ c hμ0 hμ1 hc
  have hspeed := collision_speed μ c _ x.2 h0 h1
  rcases htan with h2 | h3
  · -- vertical collision velocity: main coordinate `z₂`
    have hW : 0 < |(x : Phase) 3| := by
      apply abs_pos.2; intro h; rw [h2, h] at hspeed; simp at hspeed; linarith
    set W := |(x : Phase) 3| with hWdef
    refine ⟨(4 * (C * (C * (M + 4 * M ^ 2) + 4 * M ^ 3) + C * (M + 4 * M ^ 2) * M +
      (C * (M + 4 * M ^ 2) + 4 * M ^ 3) ^ 2) / W ^ 2 +
      2 * (M ^ 2 + (C * (M + 4 * M ^ 2) + 4 * M ^ 3) ^ 2)), by positivity,
      min 1 (W / (6 * M)), lt_min one_pos (by positivity), ?_⟩
    intro u hu0 hus
    set s := |u| with hsdef
    have hs1 : s ≤ 1 := le_trans hus (min_le_left _ _)
    have hsW : s ≤ W / (6 * M) := le_trans hus (min_le_right _ _)
    obtain ⟨cp, ca⟩ := coll_est_far μ c hμ0 hμ1 hc φ hφ M hM hM0 C hC x h0 h1 h2 s u le_rfl hs1
    obtain ⟨-, f1, -, -, -, e5⟩ := flow_estimates μ c hμ0 hμ1 hc φ hφ M hM x s u le_rfl
    rw [h1, sub_zero] at f1
    rw [h1, h0, sub_zero, abs_zero, zero_add] at e5
    have hlow : W / 2 * s ≤ |((φ u x : LeftEnergyState μ c) : Phase) 1| := by
      have h3M : 3 * M * s ≤ W / 2 := by
        have := mul_le_mul_of_nonneg_left hsW (by positivity : (0:ℝ) ≤ 3 * M)
        rw [show 3 * M * (W / (6 * M)) = W / 2 by field_simp; ring] at this; linarith
      have hwu : |(x : Phase) 3 * u| = W * s := by rw [abs_mul]
      have tri := abs_sub_abs_le_abs_sub ((x : Phase) 3 * u) (((φ u x : LeftEnergyState μ c) : Phase) 1)
      rw [abs_sub_comm] at tri
      have e5' : |((φ u x : LeftEnergyState μ c) : Phase) 1 - (x : Phase) 3 * u| ≤ 3 * M * s ^ 2 := by
        have e : (M * s + 2 * (M * s)) * s = 3 * M * s ^ 2 := by ring
        rw [← e]; exact e5
      nlinarith
    have := Vy_small_far μ M C W s (((φ u x : LeftEnergyState μ c) : Phase) 0)
      (((φ u x : LeftEnergyState μ c) : Phase) 1) (((φ u x : LeftEnergyState μ c) : Phase) 2)
      (((φ u x : LeftEnergyState μ c) : Phase) 3) hM0 hW hu0 hs1 hlow f1 ca cp
      (hC _ (φ u x).2).2.2.2 hC0.le hμ0.le hμ1.le
    rw [Vy_eq']
    exact this
  · -- horizontal collision velocity: main coordinate `z₁`
    have hW : 0 < |(x : Phase) 2| := by
      apply abs_pos.2; intro h; rw [h3, h] at hspeed; simp at hspeed; linarith
    set W := |(x : Phase) 2| with hWdef
    refine ⟨(4 * (C * (C * (M + 4 * M ^ 2) + 4 * M ^ 3) + C * (M + 4 * M ^ 2) * M +
      (C * (M + 4 * M ^ 2) + 4 * M ^ 3) ^ 2) / W ^ 2 +
      2 * (M ^ 2 + (C * (M + 4 * M ^ 2) + 4 * M ^ 3) ^ 2)), by positivity,
      min 1 (W / (6 * M)), lt_min one_pos (by positivity), ?_⟩
    ...
    have hs1 : s ≤ 1 := le_trans hus (min_le_left _ _)
    have hsW : s ≤ W / (6 * M) := le_trans hus (min_le_right _ _)
    obtain ⟨cq, cb⟩ := coll_est_near μ c hμ0 hμ1 hc φ hφ M hM hM0 C hC x h0 h1 h3 s u le_rfl hs1
    obtain ⟨f0, -, -, -, e4, -⟩ := flow_estimates μ c hμ0 hμ1 hc φ hφ M hM x s u le_rfl
    rw [h0, sub_zero] at f0
    rw [h1, h0, sub_zero, abs_zero, zero_add] at e4
    have hlow : W / 2 * s ≤ |((φ u x : LeftEnergyState μ c) : Phase) 0| := by
      have h3M : 3 * M * s ≤ W / 2 := by
        have := mul_le_mul_of_nonneg_left hsW (by positivity : (0:ℝ) ≤ 3 * M)
        rw [show 3 * M * (W / (6 * M)) = W / 2 by field_simp; ring] at this; linarith
      have hwu : |(x : Phase) 2 * u| = W * s := by rw [abs_mul]
      have tri := abs_sub_abs_le_abs_sub ((x : Phase) 2 * u) (((φ u x : LeftEnergyState μ c) : Phase) 0)
      rw [abs_sub_comm] at tri
      have e4' : |((φ u x : LeftEnergyState μ c) : Phase) 0 - (x : Phase) 2 * u| ≤ 3 * M * s ^ 2 := by
        have e : (M * s + 2 * (M * s)) * s = 3 * M * s ^ 2 := by ring
        rw [← e]; exact e4
      nlinarith
    rw [Vy_eq']
    generalize ((φ u x : LeftEnergyState μ c) : Phase) 0 = a at hlow f0 cq ⊢
    generalize ((φ u x : LeftEnergyState μ c) : Phase) 1 = b at cb ⊢
    generalize ((φ u x : LeftEnergyState μ c) : Phase) 3 = q at cq ⊢
    have hp := (hC _ (φ u x).2).2.2.1
    generalize ((φ u x : LeftEnergyState μ c) : Phase) 2 = p at hp ⊢
    rcases le_or_gt 0 a with ha | ha
    · rw [abs_of_nonneg ha] at hlow f0
      exact Vy_small_near μ M C W s a b p q hM0 hW hu0 hs1 hlow f0 cb cq hp hC0.le hμ0.le hμ1.le
    · rw [abs_of_neg ha] at hlow f0
      have cb' : |-b| ≤ (C * (M + 4 * M ^ 2) + 4 * M ^ 3) * s ^ 3 := by rw [abs_neg]; exact cb
      have cq' : |-q - μ * -a| ≤ C * (M + 4 * M ^ 2) * s ^ 2 := by
        rw [show -q - μ * -a = -(q - μ * a) by ring, abs_neg]; exact cq
      have hp' : |-p| ≤ C := by rw [abs_neg]; exact hp
      have := Vy_small_near μ M C W s (-a) (-b) (-p) (-q) hM0 hW hu0 hs1 hlow f0 cb' cq' hp' hC0.le
        hμ0.le hμ1.le
      have e : (-p * -b + -q * -a) / ((-a) ^ 2 + (-b) ^ 2) + 2 * ((-a) ^ 2 - (-b) ^ 2) - μ =
          (p * b + q * a) / (a ^ 2 + b ^ 2) + 2 * (a ^ 2 - b ^ 2) - μ := by ring
      rw [e] at this; exact this

set_option maxHeartbeats 4000000 in
/-- A far arc (below the axis, with positive horizontal velocity) that ends in a collision
arrives with Levi-Civita collision velocity `w₁ > 0 > w₂`. -/
theorem far_collision_strict (μ c : ℝ) (hμ0 : 0 < μ) (hμ1 : μ < 1)
    (hc : belowFirstCriticalValue μ c)
    (φ : Flow ℝ (LeftEnergyState μ c)) (hφ : IsLeviCivitaHamiltonianFlow μ c φ)
    (x : LeftEnergyState μ c) (T : ℝ) (hT : 0 < T)
    (hx1 : 0 < (x : Phase) 1)
    (hy : ∀ t ∈ Ioo 0 T, relativePosition μ ((φ t x : LeftEnergyState μ c) : Phase) 1 < 0)
    (hvx : ∀ t ∈ Ioo 0 T,
  ...
  have hγd : ∀ u, γ u = ((φ u x' : LeftEnergyState μ c) : Phase) := fun u => by rw [hγ]
  have hγt : ∀ u, γ u = ((φ (u + T) x : LeftEnergyState μ c) : Phase) := by
    intro u; rw [hγd, hx', ← Flow.map_add]
  have hγ0 : γ 0 = (x' : Phase) := by rw [hγd]; simp
  obtain ⟨w0, hw0⟩ : ∃ w0, w0 = (x' : Phase) 2 := ⟨_, rfl⟩
  obtain ⟨w1, hw1⟩ : ∃ w1, w1 = (x' : Phase) 3 := ⟨_, rfl⟩
  rw [← hw0, ← hw1]
  have hspeed := collision_speed μ c _ x'.2 hc0 hc1
  rw [← hw0, ← hw1] at hspeed
  have hy' : ∀ u ∈ Ioo (-T) 0, relativePosition μ (γ u) 1 < 0 := by
    intro u hu; rw [hγt]; exact hy (u + T) ⟨by linarith [hu.1], by linarith [hu.2]⟩
  -- signs along the arc
  have hz1 : ∀ u ∈ Ioo (-T) 0, 0 < γ u 1 := by
    intro u hu
    rw [hγt]
    by_contra hneg
    push Not at hneg
    have hcont : ContinuousOn (fun t => ((φ t x : LeftEnergyState μ c) : Phase) 1) (Icc 0 (u + T)) :=
      fun t _ => ((flow_coords' μ c φ hφ x t).2.1).continuousAt.continuousWithinAt
    have h0 : ((φ 0 x : LeftEnergyState μ c) : Phase) 1 = (x : Phase) 1 := by simp
    have hmem : (0:ℝ) ∈ Icc ((fun t => ((φ t x : LeftEnergyState μ c) : Phase) 1) (u + T))
        ((fun t => ((φ t x : LeftEnergyState μ c) : Phase) 1) 0) :=
      ⟨hneg, by simp only; rw [h0]; exact hx1.le⟩
    obtain ⟨t, ht, ht0⟩ := intermediate_value_Icc' (by linarith [hu.1]) hcont hmem
    simp only at ht0
    have ht' : t ≠ 0 := by rintro rfl; rw [h0] at ht0; linarith
    have := hy t ⟨lt_of_le_of_ne ht.1 (Ne.symm ht'), by linarith [ht.2, hu.2]⟩
    rw [(relPos_eq μ _).2, ht0] at this
    simp at this
  have hz0 : ∀ u ∈ Ioo (-T) 0, γ u 0 < 0 := by
    intro u hu
    have h := hy' u hu; rw [(relPos_eq μ _).2] at h
    have := hz1 u hu
    by_contra h'; push Not at h'; nlinarith
  -- first order expansion at the collision
  have est : ∀ s, 0 ≤ s → |γ (-s) 0 + w0 * s| ≤ 3 * M * s ^ 2 ∧
      |γ (-s) 1 + w1 * s| ≤ 3 * M * s ^ 2 := by
    intro s hs
    obtain ⟨-, -, -, -, e4, e5⟩ := flow_estimates μ c hμ0 hμ1 hc φ hφ M hM x' s (-s)
      (by rw [abs_neg, abs_of_nonneg hs])
    rw [hc0, hc1, ← hw0, ← hγd] at e4
    rw [hc0, hc1, ← hw1, ← hγd] at e5
    simp only [abs_zero, sub_zero, zero_add, mul_neg, sub_neg_eq_add] at e4 e5
    have e : (M * s + 2 * (M * s)) * s = 3 * M * s ^ 2 := by ring
    rw [e] at e4 e5
    exact ⟨e4, e5⟩
  have small : ∀ κ > 0, ∃ s, 0 < s ∧ s < T ∧ 3 * M * s ^ 2 < κ * s := by
    intro κ hκ
    refine ⟨min (T / 2) (κ / (6 * M)), by positivity, by
      have := min_le_left (T / 2) (κ / (6 * M)); linarith, ?_⟩
    set s := min (T / 2) (κ / (6 * M))
  ...
  have hw0nn : 0 ≤ w0 := by
    by_contra h; push Not at h
    obtain ⟨s, hs0, hsT, hsm⟩ := small (-w0) (by linarith)
    have := (abs_le.1 (est s hs0.le).1).1
    have := hz0 (-s) ⟨by linarith, by linarith⟩
    nlinarith
  have hw1np : w1 ≤ 0 := by
    by_contra h; push Not at h
    obtain ⟨s, hs0, hsT, hsm⟩ := small w1 h
    have := (abs_le.1 (est s hs0.le).2).2
    have := hz1 (-s) ⟨by linarith, by linarith⟩
    nlinarith
  suffices htan : ¬ (w0 = 0 ∨ w1 = 0) by
    push Not at htan
    exact ⟨lt_of_le_of_ne hw0nn (Ne.symm htan.1), lt_of_le_of_ne hw1np htan.2⟩
  intro htan
  have htan' : (x' : Phase) 2 = 0 ∨ (x' : Phase) 3 = 0 := by rw [← hw0, ← hw1]; exact htan
  obtain ⟨K, hK0, s1, hs10, hVy⟩ := coll_Vy_small μ c hμ0 hμ1 hc φ hφ x' hc0 hc1 htan'
  have hVy' : ∀ u, 0 < |u| → |u| ≤ s1 → |Vy μ (γ u)| ≤ K * |u| := by
    intro u h1 h2; rw [hγd]; exact hVy u h1 h2
  have hPpos : ∀ u ∈ Ioo (-T) 0, 0 < zNormSq (γ u) := by
    intro u hu; unfold zNormSq; have := hz1 u hu; positivity
  have hVx : ∀ u ∈ Ioo (-T) 0, 0 < Vx (γ u) := by
    intro u hu
    have := hvx (u + T) ⟨by linarith [hu.1], by linarith [hu.2]⟩
    rw [(jv_eq μ _).1] at this; rw [hγt]; exact this
  have hVyd : ∀ u ∈ Ioo (-T) 0, HasDerivAt (fun v => Vy μ (γ v))
      (4 * zNormSq (γ u) * (2 * Vx (γ u) + Omy μ (γ u))) u := by
    intro u hu
    have := vy_hasDerivAt μ c φ hφ x' u (by rw [← hγd]; exact hPpos u hu)
    have hfun : (fun v => Vy μ (γ v)) = fun v => Vy μ ((φ v x' : LeftEnergyState μ c) : Phase) := by
      funext v; rw [hγd]
    rw [hfun, hγd]; exact this
  have hVymono : StrictMonoOn (fun v => Vy μ (γ v)) (Ioo (-T) 0) := by
    apply strictMonoOn_of_deriv_pos (convex_Ioo _ _)
      (fun u hu => (hVyd u hu).continuousAt.continuousWithinAt)
    intro u hu
    rw [interior_Ioo] at hu
    rw [(hVyd u hu).deriv]
    have hyu : 4 * γ u 0 * γ u 1 < 0 := by
      have := hy' u hu; rwa [(relPos_eq μ _).2] at this
    have := omega_y_pos μ c hμ0 hμ1 hc (γ u) (by rw [hγd]; exact (φ u x').2) hyu
    have := hVx u hu
    have := hPpos u hu
    positivity
  have hVyneg : ∀ u ∈ Ioo (-T) 0, Vy μ (γ u) < 0 := by
    intro u hu
    by_contra hle; push Not at hle
    set u0 := u / 2 with hu0def
    have hu0 : u0 ∈ Ioo (-T) 0 := ⟨by linarith [hu.1, hu.2], by linarith [hu.2]⟩
    have hlt := hVymono hu hu0 (by linarith [hu.2])
    simp only at hlt
    set δ0 := Vy μ (γ u0)
    have hδ0 : 0 < δ0 := by linarith
    set t' := min (-u0 / 2) (min s1 (δ0 / (2 * K)))
    have ht'0 : 0 < t' := lt_min (by linarith [hu0.2]) (lt_min hs10 (div_pos hδ0 (by positivity)))
    have ht'u : t' ≤ -u0 / 2 := min_le_left _ _
    have ht's : t' ≤ s1 := le_trans (min_le_right _ _) (min_le_left _ _)
    have ht'δ : t' ≤ δ0 / (2 * K) := le_trans (min_le_right _ _) (min_le_right _ _)
    have hmt : -t' ∈ Ioo (-T) 0 := ⟨by linarith [hu0.1, hu0.2], by linarith⟩
    have hlt' : u0 < -t' := by linarith [hu0.2]
    have h1 := hVymono hu0 hmt hlt'
    simp only at h1
    have h2 := hVy' (-t') (by rw [abs_neg, abs_of_pos ht'0]; exact ht'0)
      (by rw [abs_neg, abs_of_pos ht'0]; exact ht's)
    rw [abs_neg, abs_of_pos ht'0] at h2
    have h3 : K * t' ≤ δ0 / 2 := by
      have := mul_le_mul_of_nonneg_left ht'δ hK0.le
      rw [show K * (δ0 / (2 * K)) = δ0 / 2 by field_simp] at this; exact this
    have := (abs_le.1 h2).2
    linarith
  -- hence the height decreases into the collision, contradicting `x₂ < 0`
  have hyd : ∀ u, HasDerivAt (fun v => relativePosition μ (γ v) 1) (Yd μ (γ u)) u := by
    intro u
    have hfun : (fun v => relativePosition μ (γ v) 1) =
        fun v => relativePosition μ ((φ v x' : LeftEnergyState μ c) : Phase) 1 := by
      funext v; rw [hγd]
    rw [hfun, hγd]; exact y_hasDerivAt μ c φ hφ x' u
  have hyanti : StrictAntiOn (fun v => relativePosition μ (γ v) 1) (Icc (-(T / 2)) 0) := by
    apply strictAntiOn_of_deriv_neg (convex_Icc _ _)
      (fun u _ => (hyd u).continuousAt.continuousWithinAt)
    intro u hu
    rw [interior_Icc] at hu
    rw [(hyd u).deriv]
    have huT : u ∈ Ioo (-T) 0 := ⟨by linarith [hu.1], hu.2⟩
    have hP := hPpos u huT
    unfold zNormSq at hP
    rw [Yd_eq μ _ hP.ne']
    have := hVyneg u huT
    have : 0 < 4 * (γ u 0 ^ 2 + γ u 1 ^ 2) := by positivity
    nlinarith
  have := hyanti ⟨by linarith, by linarith⟩ ⟨by linarith, le_refl _⟩
    (by linarith : -(T / 4) < (0:ℝ))
  simp only at this
  have h0 : relativePosition μ (γ 0) 1 = 0 := by
    rw [(relPos_eq μ _).2, hγ0, hc0]; ring
  rw [h0] at this
  have := hy' (-(T / 4)) ⟨by linarith, by linarith⟩
  linarith
...
theorem solution (μ c : ℝ)
  ...
  far_collision_strict μ c hμ0 hμ1 hc φ hφ x T hT hx1 hy hvx hc0 hc1
